...
0
|| checktplrefresh('./template/fxiaomi/member/login.htm', './template/default/common/seccheck.htm', 1519653538, '2', './data/template/2_2_member_login.tpl.php', './template/fxiaomi', 'member/login')
;?><?php include template('common/header'); ?><link href="template/fxiaomi/style/css/login.css" type="text/css" rel="stylesheet"/><?php $loginhash = 'L'.random(4);?><?php if(empty($_GET['infloat'])) { ?>
<div id="ct" class="wrap">
...
<div class="layout" id="main_message">
...
<a onclick="history.back()" class="back"> <span class="login_header_back"></span> 返回</a>
<img src="<?php echo $_G['style']['styleimgdir'];?>/loginlogo.png">
<?php if(!$secchecklogin2) { ?>
<h3 class="header_tit_txt">帐号登录</h3>
<?php } else { ?>
<h3 class="header_tit_txt">请输入验证码后继续登录</h3>
<?php } ?>
</div>
<div>
...
<select name="loginfield" width="45" id="loginfield_<?php echo $loginhash;?>" style="border: none">
...
<td><label for="cookietime_<?php echo $loginhash;?>" style="line-height: 38px;"><input type="checkbox" style="width: auto;display: inline" class="pc" name="cookietime" id="cookietime_<?php echo $loginhash;?>" tabindex="1" value="2592000" <?php echo $cookietimecheck;?> />自动登录</label></td>
</tr>
</table>
</div>
<!--<div class="">-->
